...
namespace Sorteio.Domain.IRepository
{
    public interface ISorteiosRepository : IRepositoryBase<Models.EntityDomain.Sorteio>
    {
        ...
        Task<PedidoRetorno> CadastrarNumerosEscolhidos(decimal valorTotal, IEnumerable<NumeroEscolhido> numeroSorteios, int idUsuario, int idSorteio);
        ...
        Task<IEnumerable<Sorteio.Domain.Models.EntityDomain.Pedido>> ObterTodosNumerosPagos(int IdPedido);
        Task<int> CancelarReserva(int idPedido);
        Task<IEnumerable<Models.EntityDomain.PedidoCliente>> ObterTodosSorteioAtivosPorID(int sorteioID);
        Task<PedidoReservadoBody> BuscarPedidoReservado(int IdPedido);
