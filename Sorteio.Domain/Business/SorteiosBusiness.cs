...
namespace Sorteio.Domain.Business
{
    public class SorteiosBusiness : BusinessBase<Models.EntityDomain.Sorteio>, ISorteiosBusiness
    {
        ...
        //    => await _sorteiosRepository.GetAllAsync(x => x.status == true);
        public async Task<IEnumerable<Models.EntityDomain.Sorteio>> ObterTodosSorteioAtivos()
            => await _sorteiosRepository.ObterTodosSorteioAtivos();
        
        public async Task<IEnumerable<Models.EntityDomain.PedidoCliente>> ObterTodosSorteioAtivosPorID(int sorteioID)
            => await _sorteiosRepository.ObterTodosSorteioAtivosPorID(sorteioID);
        ...
        public Task<int> CancelarReserva(int idPedido)
            => _sorteiosRepository.CancelarReserva(idPedido);

        public Task<PedidoReservadoBody> BuscarPedidoReservado(int IdPedido)
            => _sorteiosRepository.BuscarPedidoReservado(IdPedido);
    }
}
