...
namespace Sorteio.Controllers
{
    public class HomeController : Controller
    {
        ...
        public HomeController(
            ISorteiosBusiness sorteiosBusiness, 
            ICategoriaSorteioBusiness categoriaSorteioBusiness, 
            IFormasDePagamentoBusiness formasDePagamentoBusiness)
        {
        ...
        /// <summary>
        /// Passa o Numero do Pedido
        /// </summary>
        /// <param name="idSorteio"></param>
        /// <returns></returns>
        [HttpGet]
        //[Route("[controller]/[action]/{reservafinish:int}")]
        [Route("reservafinish/{pedido:int}")]
        public async Task<IActionResult> ReservaFinish(int pedido)
        {
            var ped  = await _sorteiosBusiness.BuscarPedidoReservado(pedido);
            if (!(ped.pedido.ped_guid is null))
                ped.pedido.QRCodeURL = "https://localhost:5001/downloads/qrcode/" + ped.pedido.ped_guid + ".png";
            var forma = await _formasDePagamentoBusiness.ObterTodasFormasDePagamentoAtivo();
            // Multiplos Objetos
            dynamic mymodel = new ExpandoObject();
            mymodel.Pedido = ped.pedido;
            mymodel.Forma = forma;

            //return View(ViewBag.PedidoData);
            return View(mymodel);
        }
