...
namespace Sorteio.Controllers
{
    public class SorteiosController : Controller
    {
        ...
        [HttpGet]
        [Route("[controller]/[action]/{idPedido:int}")]
        public async Task<JsonResult> CancelarReserva(int idPedido)
        {
            var resultado = await _sorteiosBusiness.CancelarReserva(idPedido);

            if (resultado == 1)
                return Json(new { erro = false, mensagem = "Reserva Cancelada com sucesso!" });
            else
                return Json(new { erro = false, mensagem = "Erro ao Cancelar Reserva!" });
        }
        ...
        [HttpGet]
        [Route("[controller]/[action]/{idSorteio:int}")]
        public async Task<JsonResult> ObterTodosSorteioAtivosPorID(int idSorteio)
        {
            var resultado = await _sorteiosBusiness.ObterTodosSorteioAtivosPorID(idSorteio);
            return Json(resultado);
        }
    }
}